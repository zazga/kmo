...
func New(cfg config.Config, token string, keyStore keychain.Store, infoLog, errLog *slog.Logger) Model {
	...
	m := Model{
		...
		chat:               ui.NewChat(),
		compose:            ui.NewCompose(),
		focus:              ui.FocusSidebar,
		connection:         "offline",
		telegramConnection: telegramStatus,
		daemonStatuses:     statuses,
		users:              make(map[string]*model.User),
	}
	telegramToken, _ := keychain.Telegram().Get()
	if cfg.SetupComplete() && strings.TrimSpace(token) != "" {
		m.service = mm.New(cfg.ServerURL, token, infoLog, errLog)
		m.mode = modeConnecting
	} else {
		m.mode = modeSetup
		m.setup = ui.NewSetup(cfg.ServerURL, token, cfg.Telegram.Enabled, telegramToken)
	}
	return m
}
...
func (m Model) Init() tea.Cmd {
	...
	if m.mode == modeConnecting && m.service != nil {
		cmds = append(cmds, bootstrapCmd(m.ctx, m.service))
	}
...
func bootstrapCmd(ctx context.Context, svc *mm.Service) tea.Cmd {
	return func() tea.Msg {
...
func waitEventCmd(svc *mm.Service) tea.Cmd {
	...
	return func() tea.Msg {
		ev, ok := <-svc.Events()
		if !ok {
			return websocketMsg{event: mm.Event{Kind: mm.EventOffline}}
		}
		return websocketMsg{event: ev}
	}
}
...
func waitDaemonStatusCmd(ch <-chan daemon.Status) tea.Cmd {
	...
	return func() tea.Msg {
		status, ok := <-ch
		if !ok {
			return daemonStatusMsg{status: daemon.Status{Daemon: "unavailable", Telegram: "unavailable"}}
		}
